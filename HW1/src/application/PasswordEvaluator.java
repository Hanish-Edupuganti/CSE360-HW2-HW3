	...
	public static String evaluatePassword(String input) {
		...
		while (running) {
			...
			if (currentChar >= 'A' && currentChar <= 'Z') {
				...
				//REMOVED EXTRA SPECIAL CHARCTERS FROM THE ABOVE LIST 
		...
		String errMessage = "";
		if (!foundUpperCase)
			errMessage += "Password must have atleast one upper case letter\n";
		//CHANGED THE ERROR PROMPT TO HELP THE USER TO ENTERR ATLEAST ONE UPPER CASE LETTER 
		
		if (!foundLowerCase)
			errMessage += "Password must have atleast one lower case letter\n";
		//CHANGED THE ERROR PROMPT TO HELP THE USER TO ENTERR ATLEAST ONE LOWER CASE LETTER 

		
		if (!foundNumericDigit)
			errMessage += "Password must have atleast one number\n";
		//CHANGED THE ERROR PROMPT TO HELP THE USER TO ENTERR ATLEAST ONE NUMBER  

			
		if (!foundSpecialChar)
			errMessage += "Password must have atleast one special character\n";
		//CHANGED THE ERROR PROMPT TO HELP THE USER TO ENTERR ATLEAST ONE SPECIAL CHARACTER  

			
		if (!foundLongEnough)
			errMessage += "Password miust be atleast 8 or more characters long\n";
		//CHANGED THE ERROR PROMPT TO HELP THE USER TO ENTERR A PASSWORD WHICH IS LONG ENOUGH 
		...
		passwordIndexofError = currentCharNdx;
		return errMessage;
		// REMOVED THE EXTRA ERROR PROMPT 
