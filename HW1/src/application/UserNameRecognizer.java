...
public class UserNameRecognizer {
	...
	public static String checkForValidUserName(String input) {
		...
		while (running) {
			...
			case 0: 
				...
				if ((currentChar >= 'A' && currentChar <= 'Z' ) ||		// Check for A-Z
					...
					// REMOVED THE CHECKING FOR NUMBERS (0-9) CONDITION
					nextState = 1;
			...
			case 1: 
				...
				(currentChar == '_') ||
				(currentChar == '-'))
					//ADDED - AND _ TO BE CHECKED UNDER SPECIAL CHARACTERS CATEGORY
				{							// Check for /
					nextState = 2;
		...
		case 0:
			...
			//REMOVED THE (0-9) PARAMETER SO THAT THE USER CAN NO LONGER ENTER ANY NUMBER AS THE FIRST CHARACTER
		...
		case 1:
			...
			if (userNameSize < 4) {
				...
				userNameRecognizerErrorMessage += 
					"A UserName character may only contain the characters A-Z, a-z, 0-9, special characters('.', _, -)\n";
				//CHANGED THE PROMPT TO THE USER SO THAT THEY KNOW A USERNAME CAN CONTAIN SPECIAL CHARACTERS AS WELL
				return userNameRecognizerErrorMessage;
			}
			else {
					// UserName is valid
		...
		case 2:
			...
			//CHANGED THE PROMPT SO THAT IT LETS THE USER KNOW THAT AFTER ANY SPECIAL CHARACTER IT SHOULD BE OTHER CHARACTERS
			return userNameRecognizerErrorMessage;
